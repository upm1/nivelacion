...
createElement("button", "btn btn-danger float-right clearAll px-5", container, clear => {
  clear.textContent = "limpiar mi carrito";
  clear.addEventListener("click", (event) => {
    dbcart.delete().then(() => {
      console.log("Database borrada");
      dbcart.close();
      location.reload();
    }).catch((err) => {
      console.error("No se pudo borrar la base de datos");
    })
  });
});
            ...
            createElement("div", "col-md-8", newRow, innercol8 => {
              ...
              createElement("div", "card-body", innercol8, cartbody => {
                ...
                createElement(
                  ...
                  p => {
                    // paragraph with text
                    p.textContent = "producto agregado al carrito";
                  }
                );
